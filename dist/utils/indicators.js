...
function calculateADX(highs, lows, closes, period = 14) {
    ...
    for (let i = 1; i < length; i++) {
        ...
        const currentTr = Math.max(highs[i] - lows[i], Math.abs(highs[i] - closes[i - 1]), Math.abs(lows[i] - closes[i - 1]));
        tr.push(currentTr);
    ...
    const getDI = (dm, trVal) => (trVal === 0 ? 0 : (dm / trVal) * 100);
...
function generateTradingViewRating(highs, lows, closes, volumes = []) {
    ...
    if (len < 200) {
        ...
        return {
            ...
            movingAverages: { rating: "NEUTRAL", buyCount: 0, sellCount: 0, neutralCount: 0, items: {} },
            volatility: { atr: 0 },
    ...
    let obvAction = "NEUTRAL";
    if (curObv > curObvSma)
        obvAction = "BUY";
    else if (curObv < curObvSma)
        obvAction = "SELL";
    ...
    let stochAction = "NEUTRAL";
    if (curStochK < 20 && curStochK > curStochD)
        stochAction = "BUY";
    else if (curStochK > 80 && curStochK < curStochD)
        stochAction = "SELL";
    if (stochAction === "BUY")
        oscBuy++;
    else if (stochAction === "SELL")
        oscSell++;
    else
        oscNeutral++;
    // CCI
    ...
    let adxAction = "NEUTRAL";
    if (curADX > 25) {
        if (curPlusDI > curMinusDI)
            adxAction = "BUY";
        else if (curMinusDI > curPlusDI)
            adxAction = "SELL";
    }
    if (adxAction === "BUY")
        oscBuy++;
    else if (adxAction === "SELL")
        oscSell++;
    else
        oscNeutral++;
    // AO
    ...
    let aoAction = "NEUTRAL";
    if (curAO > 0 && prevAO <= 0)
        aoAction = "BUY";
    else if (curAO < 0 && prevAO >= 0)
        aoAction = "SELL";
    if (aoAction === "BUY")
        oscBuy++;
    else if (aoAction === "SELL")
        oscSell++;
    else
        oscNeutral++;
    // Momentum
    ...
    let momAction = "NEUTRAL";
    if (curMom > 0 && prevMom <= 0)
        momAction = "BUY";
    else if (curMom < 0 && prevMom >= 0)
        momAction = "SELL";
    if (momAction === "BUY")
        oscBuy++;
    else if (momAction === "SELL")
        oscSell++;
    else
        oscNeutral++;
    // MACD
    ...
    if (macdAction === "BUY")
        oscBuy++;
    else if (macdAction === "SELL")
        oscSell++;
    else
        oscNeutral++;
    let oscRating = "NEUTRAL";
    if (oscBuy > oscSell && oscBuy > oscNeutral)
        oscRating = "BUY";
    else if (oscSell > oscBuy && oscSell > oscNeutral)
        oscRating = "SELL";
    // 2. Moving Averages evaluation
    let maBuy = 0, maSell = 0, maNeutral = 0;
    const evaluateMA = (maVal, currentPrice) => {
    ...
    if (totalRatingCount > 0) {
        ...
        if (buyRatio >= 0.7)
            summaryRating = "STRONG_BUY";
        else if (buyRatio >= 0.5)
            summaryRating = "BUY";
        else if (sellRatio >= 0.7)
            summaryRating = "STRONG_SELL";
        else if (sellRatio >= 0.5)
            summaryRating = "SELL";
        else
            summaryRating = "NEUTRAL";
    }
    return {
        summary: {
