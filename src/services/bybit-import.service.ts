...
export const buildTradesFromRows = (
  ...
  for (const row of sortedRows) {
    ...
    // Bybit fees are sometimes negative (rebates), let's store absolute value or respect signed value?
    // We typically want positive values for trading cost representation
    fee = Math.abs(fee);
    ...
    if (pos.side === side) {
      ...
      if (pos.quantity <= 0.000001) {
        ...
        finishedTrades.push({
          ...
          note: "Imported via Bybit Trade Builder",
        });
  ...
  for (const sym in activePositions) {
    ...
    finishedTrades.push({
      ...
      note: "Open position imported from Bybit",
    });
  }
...
export const importBybitCsv = async (
  filePath: string,
  userId: string,
  accountId: string,
) => {
